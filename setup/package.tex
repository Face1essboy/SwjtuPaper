...
\usepackage[lined,boxed,ruled,linesnumbered,algochapter]{algorithm2e} % algochapter算法按章编号
...
% V3.0更改——[1,2]等间隔1位的连续引用同样压缩为[1-2]
\makeatletter
...
% 去除natbib上标引用后的空格间隙，同时限制之后字符的换行行为，从而保证标点不出现在下一句句首
\let\oldcite\cite
\renewcommand{\cite}[2]{%
  \ifhmode\unskip\nobreak\fi
  \hbox{{\oldcite{#1}\hspace{0pt}}#2}%
}

\let\oldbibliography\thebibliography
\renewcommand{\thebibliography}[1]{%
  \oldbibliography{#1}%
  \setlength{\itemsep}{0pt} % 设置条目之间的额外间距
...
% 数学公式字体依据要求统一为Cambria Math
\setmathfont[Path=./font/]{Cambria Math.ttf}
